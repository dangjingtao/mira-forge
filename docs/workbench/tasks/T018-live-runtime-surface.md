...
- 2026-09-02 human UI review requested a focus-layer correction: keep runtime attention visible on the workbench, move full runtime inventory and raw Event Log behind keyboard-accessible inspector/modal surfaces, group Builder/Reviewer runtime separately from Main Threads, and make authoritative Batch/Task relationships explicit.
- Focus-layer correction merged through PR `#26` at squash commit `72e75a8deffb25251c09f6ae2986912935cadbbf`. The default workbench now keeps one focusable runtime summary row; full Builder/Reviewer and Main Thread inventories live in `RuntimeInspectorModal`, selected session/result/error evidence lives in `RuntimeRowDetail`, and raw events live in `RuntimeEventLogModal`.
- PR `#26` also introduced the reusable `ModalFrame` so runtime overlays contain keyboard events instead of leaking App-level TUI shortcuts behind the modal, trap Tab within the dialog, support Escape close, and restore focus to the runtime summary on close. The obsolete always-visible `LiveRuntimeSurface` component was removed instead of retained as a parallel implementation.
- PR `#26` final-head Verify run `33613098623` passed `npm test`, `npm run typecheck`, `npm run build` and `npm run smoke`. Merged-tree `dev` Verify run `33613288805` passed the same four gates again.
...
The T018 focus-layer correction is merged to `dev` with automated verification green. The default TUI keeps current work visible while secondary runtime inventory and raw events are recalled through explicit inspector surfaces. Repeat the final browser smoke with one real Builder dispatch: verify runtime summary counts, Runtime Inspector task/batch relation, truthful elapsed time, Main Thread focus, terminal Builder-result handoff and next-turn Main Thread continuation. Keep T018 in `REVIEW` until that observational smoke is accepted.
