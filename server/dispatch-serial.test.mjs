...
test('V1 keeps one active dispatch per Builder adapter even across different tasks', async () => {
  ...
  const first = await manager.dispatchTask({ batchId, taskId: 'T001', prompt: 'First task' })
  await waitFor(async () => (await store.read()).dispatches[0]?.status === 'running')

  await assert.rejects(
    () => manager.dispatchTask({ batchId, taskId: 'T002', prompt: 'Second task' }),
    /builder adapter already has an active dispatch/,
  )
  ...
  const second = await manager.dispatchTask({ batchId, taskId: 'T002', prompt: 'Second task after first completed' })
